...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('smart_auto_fill_tray_fixed.log', encoding='utf-8'),
        logging.StreamHandler()
    ]
)

class SmartAutoFillTrayFixed:
    def __init__(self):
        # 工具状态
        self.is_enabled = True  # 默认启用
        self.last_clipboard = ""
        ...
        # 配置
        self.config_file = "smart_tray_fixed_config.json"
        self.load_config()
        ...
        logging.info("智能自动填充工具（修复版托盘版）初始化完成")
    ...
    def create_tray_icon(self):
        ...
        try:
            ...
            self.tray_icon = Icon("smart_auto_fill_fixed", image, f"智能自动填充工具 - {status_text}", menu)
        except Exception as e:
    ...
    def show_status(self):
        ...
        try:
            ...
            if clipboard_content:
                print(f"剪贴板内容: {clipboard_content[:50]}{'...' if len(clipboard_content) > 50 else ''}")
            else:
                print("剪贴板为空")
        except:
            print("无法获取剪贴板内容")
    ...
    def test_fill(self):
        """测试填充功能"""
        try:
            test_text = f"修复版测试 - {time.strftime('%H:%M:%S')}"
    ...
    def manual_fill(self):
        ...
        try:
            ...
            if clipboard_content:
                pyautogui.hotkey('ctrl', 'v')
                logging.info(f"手动填充完成: {clipboard_content[:50]}{'...' if len(clipboard_content) > 50 else ''}")
            else:
                logging.info("剪贴板为空，无法填充")
        except Exception as e:
            logging.error(f"手动填充失败: {e}")
            ...
            try:
                ...
                if current_content and current_content != last_content:
                    ...
                    if (self.is_enabled and 
                        ...
                        if not self.is_enabled:
                            logging.info("功能已禁用，跳过自动填充")
                        elif not self.is_mouse_over_input:
    ...
    def start(self):
        """启动工具"""
        logging.info("智能自动填充工具（修复版托盘版）启动中...")
        ...
        logging.info("智能自动填充工具（修复版托盘版）已启动")
        logging.info(f"当前状态: {'启用' if self.is_enabled else '禁用'}")
